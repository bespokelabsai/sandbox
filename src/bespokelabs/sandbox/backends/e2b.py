"""E2B code-interpreter sandbox backend."""
    ...
    def __init__(self) -> None:
        try:
            from e2b_code_interpreter import (
                Sandbox as E2BSandbox,  # type: ignore[import-untyped]
            )
...
class E2BSession:
    ...
    def execute_code(
        ...
        del language  # Unused; the E2B code endpoint always executes Python.
        try:
            ...
            stdout = (
                "\n".join(stdout_parts)
                if isinstance(stdout_parts, list)
                else str(stdout_parts)
            )
            stderr = (
                "\n".join(stderr_parts)
                if isinstance(stderr_parts, list)
                else str(stderr_parts)
            )
            ...
            raise SandboxExecutionError(
                f"E2B code execution failed: {exc}"
            ) from exc
    ...
    def execute_command(
        ...
        try:
            ...
            raise SandboxExecutionError(
                f"E2B command execution failed: {exc}"
            ) from exc
    ...
    def download_file(self, remote_path: str, local_path: str) -> None:
        try:
            ...
            raise SandboxExecutionError(
                f"E2B download_file failed: {exc}"
            ) from exc
