...
from bespokelabs.sandbox.backends.safehouse import (
    SafehouseClient,
    SafehouseSession,
)
...
class SafehouseSessionTests(unittest.TestCase):
    ...
    def _create_session(self, **config_kwargs: object) -> SafehouseSession:
        ...
        with (
            mock.patch(
                "bespokelabs.sandbox.backends.safehouse.platform.system",
                return_value="Darwin",
            ),
            mock.patch(
                "bespokelabs.sandbox.backends.safehouse.shutil.which",
                return_value="/usr/local/bin/safehouse",
            ),
        ):
    ...
    def test_create_sets_home_and_sandbox_root(self) -> None:
        with tempfile.TemporaryDirectory() as workdir:
            session = self._create_session(
                workdir=workdir, env_vars={"FOO": "bar"}
            )
    ...
    def test_execute_code_wraps_python_with_preamble(self) -> None:
        ...
        with (
            mock.patch.object(
                session, "_resolve_interpreter", return_value="python3.13t"
            ),
            mock.patch.object(
                session, "_run", return_value=SandboxResult(stdout="ok")
            ) as run,
        ):
        ...
        self.assertEqual(
            cmd[:4],
            [
                "/usr/local/bin/safehouse",
                f"--workdir={session._workdir}",
                "--env",
                "--",
            ],
        )
    ...
    def test_execute_command_rewrites_nested_shell_redirects(self) -> None:
        ...
        with mock.patch.object(
            session, "_run", return_value=SandboxResult()
        ) as run:
